...
void csvelas(){
  ...
  json j;
  {
    std::ifstream json_input_file("elasrunlist.json");
    json_input_file >> j;
  }
 ...
 //     new TH2F("yield sim", "yield sim", 10, 6008, 6018, 100, 0, 200);
  ...
  for (json::iterator it = j.begin(); it != j.end(); ++it) {
    ...
    std::cout<<"hmsangle"<<hms_angle<<"hms_momentum"<<hms_momentum<<"charge"<<charge<<"normfac"<<normfac<<std::endl;
    ...
    //std::string rootfile_data = fmt::format("/group/c-csv/shuo/hallc_replay_sidis_fall18/ROOTfiles/"
    //                                        "coin_replay_production_{}_-1.root",
    //                                        runnumber);
    std::string rootfile_data = fmt::format("/group/c-csv/shuo/replay/ROOTfiles/"
                                            "coin_replay_production_{}_-1.root",runnumber);
    ...
    std::cout<<"yield data before cut"<<yield_data_before<<std::endl;
   ...
   .Filter([](double etottracknorm){return etottracknorm>0.8;},{"P.cal.etottracknorm"})
    ...
    std::cout<<"data yield"<<yield_data_after<<std::endl;
   // h_yield_aftercut->Fill(runnumber,yield_data_after);
    ...
    G_yield_data_angle->SetPoint(i,hms_angle+0.05,yield_data_after);
    ...
    auto hWeight = d1sim.Histo1D({"weight","weight",100,-10,10},"weight");
    auto *cweight = new TCanvas;
    hWeight->DrawCopy();
    ...
    auto d0sim = d1sim.Filter(
        [Wmax, Wmin](float w) { return w < Wmax && w > Wmin; }, {"W"});
    //.Filter([dpcut](float dp){return dp<dpcut;},{"hsdelta"});
    //.Filter([](float EOVerP){return EOverP>0.8},{""});

    std::cout<<"sim counts"<<*d1sim.Count()<<std::endl;
    ...
    std::cout<<"yield_sime"<<yield_sim<<std::endl;
    ...
    G_yield_sim_angle->SetPoint(i,hms_angle,yield_sim);
